...
<?php 
 ...
 ?>
	...
	<body>
		...
		<!-- /Header -->
		<?php $req= $bdd->prepare("SELECT * FROM T_comment, T_user, T_archive_comment WHERE T_comment.comment_id = T_archive_comment.comment_id AND T_user.user_id = T_archive_comment.user_id");
							$req->execute();
							//boucle pour tout afficher
							while($donnees = $req->fetch()) { ?>
		<div><?php echo $donnees['user_login']?> a archivé le commentaire "<?php echo $donnees ['comment_content']?>" le <?php echo $donnees['archive_comment_date']?><a href='./traitement/restore_comment.php?id=<?php echo $donnees['comment_id']?>'>X</a><div>
							<?php } ?>
		<!-- Footer -->
		<footer id="footer">
			<!-- container -->
